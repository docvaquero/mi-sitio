...
(function(){
  ...
  (function(){
    ...
    btn.addEventListener('click', () => {
      ...
      header.classList.toggle('is-open', open);      // para ocultar brand y dibujar la “X”
      document.body.classList.toggle('no-scroll', open); // bloquea scroll detrás
...
(function(){
  ...
  (function(){
    ...
    viewport.addEventListener('wheel', (e) => {
      // si el gesto ya es horizontal, no tocamos nada
      if (Math.abs(e.deltaY) <= Math.abs(e.deltaX)) return;

      const before = viewport.scrollLeft;
      viewport.scrollLeft += e.deltaY;

      // ¿se movió realmente el carrusel?
      const moved = viewport.scrollLeft !== before;

      // ¿estamos en un borde?
      const atStart = viewport.scrollLeft <= 0;
      const atEnd   = viewport.scrollLeft >= (track.scrollWidth - viewport.clientWidth - 1);

      // solo cancelamos si estamos desplazando en medio del carrusel
      if (moved && !atStart && !atEnd) {
        e.preventDefault();
      }
    }, { passive:false });
