...
(() => {
  ...
  /* Parallax suave (solo si no prefiere reducir movimiento) */
  (() => {
    if (mm('(prefers-reduced-motion: reduce)').matches) return;
    const secs = $$('[data-parallax]'); if (!secs.length) return;
    const move = (el, dx, dy) => {
    ...
    secs.forEach(sec => {
      ...
      sec.addEventListener('mouseleave', () => {
        sec.style.setProperty('--tx','0px'); sec.style.setProperty('--ty','0px');
      });
    });
  })();
